...
const i18n = {
  ...
  ja: {
    // (optional) provide translations; EN is fallback
  }
};
...
const stepsScroller = document.getElementById("stepsLabels");
...
function showStep(nextIdx){
  ...
  // keep active label in view on small screens
  const active = stepsLabels[nextIdx];
  if (active && stepsScroller) {
    const left = active.offsetLeft - 12;
    stepsScroller.scrollTo({ left, behavior: "smooth" });
  }
...
function closeMenu(){ moreMenu.classList.remove("open"); moreBtn.setAttribute("aria-expanded","false"); moreMenu.setAttribute("aria-hidden","true"); }
function openMenu(){ moreMenu.classList.add("open"); moreBtn.setAttribute("aria-expanded","true"); moreMenu.setAttribute("aria-hidden","false"); }
moreBtn.addEventListener("click", (e)=>{
  e.stopPropagation();
  if (moreMenu.classList.contains("open")) closeMenu(); else openMenu();
});
document.addEventListener("click", (e)=>{ if (!moreMenu.contains(e.target) && e.target!==moreBtn) closeMenu(); });
document.getElementById("menuPrint").addEventListener("click", ()=>{ closeMenu(); window.print(); });
document.getElementById("menuExport").addEventListener("click", ()=>{ closeMenu(); downloadJSON(collectPayload()); });
...
// set initial progress width based on steps
