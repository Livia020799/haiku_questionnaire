...
const i18n = {
  en: {
    ...
    likertHint: "from 1 (very low / disagree) to 7 (very high / agree)",
  ...
  ja: {
    // optional translations (EN fallback)
  }
};
...
function attachKirejiLogic(card){
  ...
  function sync(){
    if (unsure.checked){
      ...
      // keep text input but it won’t be selected when unsure is on
    } else {
  ...
  checks.forEach(c => c.addEventListener('change', ()=>{
    if (c !== unsure && c.checked) unsure.checked = false;
    sync();
  }));
  sync();
}
...
function onFinish(){
  // just go to Done (no auto-download)
  showStep(6);
}
